...
// made with love for gamkedo by mcfunkypants
    ...
    if (!decalPositions) {
      ...
      for (let i = 0; i < length; i++) {
        if (decalPositions[i].alpha > 0) {
            drawImageRotatedAlpha(canvasContext, decalPositions[i].img, decalPositions[i].x, decalPositions[i].y, decalPositions[i].angle, decalPositions[i].alpha);
        }
      }
}
